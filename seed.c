...
int64_t mb_anchor_meth_flt(void *km, const l2b_t *l2b, l2b_meth_t mt, int32_t qlen, const uint8_t *seq4, int64_t n_a, mb_anchor_t *a)
{
	...
	int32_t j, max_len = 0;
	uint8_t *tseq;
	if (mt == L2B_METH_NONE) return n_a;
	for (i = 0; i < n_a; ++i)
		if (a[i].len > max_len) max_len = a[i].len;
	tseq = Kmalloc(km, uint8_t, max_len);
	for (i = 0; i < n_a; ++i) {
		mb_anchor_t *ai = &a[i];
		int32_t qs = ai->sid&1? qlen - (ai->qpos + 1) : ai->qpos + 1 - ai->len;
		l2b_getseq(l2b, ai->sid>>1, ai->tpos + 1 - ai->len, ai->tpos + 1, tseq);
		if (ai->sid&1) kom_revcomp(ai->len, (char*)tseq);
		if (mt == L2B_METH_C2T) {
			for (j = 0; j < ai->len; ++j)
				if (tseq[j] == 3 && seq4[qs + j] == 1)
					break;
			if (j != ai->len) ai->flt = 1;
		} else if (mt == L2B_METH_G2A) {
			for (j = 0; j < ai->len; ++j)
				if (tseq[j] == 0 && seq4[qs + j] == 2)
					break;
			if (j != ai->len) ai->flt = 1;
		}
		#if 0
		fprintf(stderr, "==> %lld:%d\n", i, ai->flt);
		for (j = 0; j < ai->len; ++j) fputc("ACGTN"[tseq[j]], stdout); fputc('\n', stdout);
		for (j = 0; j < ai->len; ++j) fputc("ACGTN"[seq4[qs+j]], stdout); fputc('\n', stdout);
		#endif
	}
	kfree(km, tseq);
	for (i = n = 0; i < n_a; ++i)
